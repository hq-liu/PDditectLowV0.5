		 ...
		 if((i < envelope_upl[0]) || i > envelope_upl[ArrayLen-2])   //在最左右两点以外可以考虑为没有限制
			...
			for(k = 0;k < ArrayLen ; k=k+2) 
				{ 
				  ...
				  {
						if(envelope_upl[k]!=envelope_upl[k+2])
							y_up[i] = i * line_k[k/2] + line_b[k/2]; //y=kx+b计算包络线的离散y值
						else if(envelope_upl[k+1]>envelope_upl[k+3])//y1>y2
							y_up[i] = envelope_upl[k+1];
						else if(envelope_upl[k+1]<envelope_upl[k+3])//y1<y2
							y_up[i] = envelope_upl[k+3];
					}
	      }
			}
	 }
		 ...
		 if((i< envelope_below[0]) || i > envelope_below[ArrayLen-2])   //在最左右两点以外可以考虑为没有限制
			...
			for(k = 0;k < ArrayLen ; k=k+2) 
				{ 
				  ...
				  {
						if(envelope_belowl[k] != envelope_belowl[k+2])
								y_below[i] = i * line_k[k/2] + line_b[k/2]; //y=kx+b计算包络线的离散y值
						else if(envelope_belowl[k+1] < envelope_belowl[k+3]) //y1<y2
								y_below[i] = envelope_belowl[k+1];
						else if(envelope_belowl[k+1] > envelope_belowl[k+3])  //y1>y2
								y_below[i] = envelope_belowl[k+3];
				  }
	      }
	 }
	}
