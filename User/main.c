  ...
  while(1)
	{		
		 if(Uart3_Rx_F)  //如果有命令参数需要配置
		 {
			...
			{
					if((save_line[DataPointer]&0xFF) == 0x55) //低八位为0x55
					{
						SendData[cntt] = 0xAA;cntt++;
						SendData[cntt] = 0x5A;cntt++;
					}
					else if((save_line[DataPointer]&0xFF) == 0xAA) //低八位为0xAA
					{
						SendData[cntt] = 0xAA;cntt++;
					...
					if(((save_line[DataPointer]&0xFF00)>>8) == 0x55)//高八位为0x55
					{
						SendData[cntt] = 0xAA;cntt++;
						SendData[cntt] = 0x5A;cntt++;
					}
					else if(((save_line[DataPointer]&0xFF00)>>8) == 0xAA)//高八位为0x55
					{
						SendData[cntt] = 0xAA;cntt++;
						SendData[cntt] = 0xA5;cntt++;
					}
