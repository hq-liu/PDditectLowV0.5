...
{ 
	if((TIM5->SR & 0x01) && (TIM5->DIER & 0x01))//溢出中断 
	{
		if(Tim5State==0)  //S1阶段进入S2阶段
		{
				...
				//数据长度，根据DataLen得出，分为低八位和高八位发送
				if((DataLen&0XFF)==0x55) {DataHead[HeadCnt]=0xAA;HeadCnt++;DataHead[HeadCnt]=0x5A;HeadCnt++;}
				else if((DataLen&0XFF)==0xAA) {DataHead[HeadCnt]=0xAA;HeadCnt++;DataHead[HeadCnt]=0xA5;HeadCnt++;}
				else {DataHead[HeadCnt]=DataLen&0XFF;HeadCnt++;}
				if(((DataLen&0XFF00)>>8)==0x55) {DataHead[HeadCnt]=0xAA;HeadCnt++;DataHead[HeadCnt]=0x5A;HeadCnt++;}
				else if(((DataLen&0XFF00)>>8)==0xAA) {DataHead[HeadCnt]=0xAA;HeadCnt++;DataHead[HeadCnt]=0xA5;HeadCnt++;}
				else {DataHead[HeadCnt]=(DataLen&0XFF00)>>8;HeadCnt++;}
				...
				if(((SerialNumber&0XFF00)>>8)==0x55) {DataHead[HeadCnt]=0xAA;HeadCnt++;DataHead[HeadCnt]=0x5A;}
				else if(((SerialNumber&0XFF00)>>8)==0xAA) {DataHead[HeadCnt]=0xAA;HeadCnt++;DataHead[HeadCnt]=0xA5;}
				else {DataHead[HeadCnt]=(SerialNumber&0XFF00)>>8;}
				for(s=0;s<HeadCnt-1;s++) //把DataHead数组的内容发送出去
				{
		...
		else if(Tim5State==1) //S2状态切换到S3状态
		{
				...
				else if(flag_ng) {DataTail[TailCnt]=0x01;TailCnt++;}
				for(s=0;s<HeadCnt;s++)
